...
| `Shared/Events/*` (e.g. `CacheEvents`) | `BaseEvent` ✅ **RESTORED (G8)** → `Events\Events` (Dispatchable + SerializesModels; renamed from BaseEvent in G12c). `CacheEvents` is now a real event built on the base; the other never-dispatched stubs stay **dropped**. | `Events\Events`; define concrete events natively. | (in-repo) `src/Events/Events.php` |
