...
| `Support/Utilities/Auth.php` | ✅ **RESTORED (G6)** → `Utilities\AuthUtil` (typed guard accessor; pheg removed). | `Utilities\AuthUtil` / native `auth()`. | (in-repo) `src/Utilities/AuthUtil.php` |
...
| `Laravel/Jobs/BaseJob` | ✅ **RESTORED (G8)** — reusable abstract `ShouldQueue` base (tries/backoff/timeout + `failed()` logging via `LoggingUtil`). | `Jobs\BaseJob`. | (in-repo) `src/Jobs/BaseJob.php` |
