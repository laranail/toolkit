...
class RequirementsDiagnosticsTest extends TestCase
{
    ...
    {
        // Pin the thresholds so the outcome is deterministic regardless of the
        // runner's actual free space / usage (warnAtPercent 100 never trips
        // short of a 100%-full disk).
        $result = $this->diagnostics->diskSpace([sys_get_temp_dir()], minMb: 1, recommendedMb: 1, warnAtPercent: 100);
    ...
    public function test_disk_space_probe_reports_low_below_recommendation(): void
    {
        $result = $this->diagnostics->diskSpace(
            ...
            // Never warn (short of a 100%-full disk) so the status is driven by
            // the recommendation gap alone — deterministic on any runner.
            warnAtPercent: 100,
        );
